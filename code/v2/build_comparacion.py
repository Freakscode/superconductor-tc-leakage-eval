...
_INPUTS = ["tabla2_shuffle.csv", "tabla2_noshuffle_control.csv", "tabla2_crecimiento_desviacion.csv",
           "tabla2_control_reproduccion.csv", "tabla2_dispersion_entre_folds.csv",
           "tabla2_rango_inflacion.json", "tabla2_determinismo_groupkfold.json"]
if "--help" in sys.argv or "-h" in sys.argv or "--dry-run" in sys.argv:
    print(__doc__); print(f"ROOT={ROOT}\nOUT={OUT}")
    for f in _INPUTS + [os.path.join("..", "results", "repeated_seed_intervals.csv")]:
        p = Path(R(f)); print(f"  {'ok ' if p.exists() else 'MISSING'} {p.resolve()}")
    print("usage: python build_comparacion.py [--dry-run]   (writes results_v2/tabla2_comparacion.md)")
    sys.exit(0)
...
_pubcsv = pd.read_csv(ROOT / "results" / "repeated_seed_intervals.csv")
