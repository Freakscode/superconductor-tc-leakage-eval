...
DATA_DIR = ROOT / "data"        # (DATA below is this script's in-memory cache dict)
...
def main():
    if "--help" in sys.argv or "-h" in sys.argv or "--dry-run" in sys.argv:
        ...
        for p in (CODE / "tc_pipeline.py", CODE / "datasetB_pipeline.py", DATA_DIR / "train.csv",
                  DATA_DIR / "unique_m.csv", DATA_DIR / "datasetB_featurized.npz"):
            print(f"  {'ok ' if p.exists() else 'MISSING'} {p}")
        print("usage: python run_tabla2_shuffle.py [--paso1 [--solo-paso1]] [--dry-run]   "
              "(full run: 60 CV tasks, ~45 min; NW=<threads>)")
        return
