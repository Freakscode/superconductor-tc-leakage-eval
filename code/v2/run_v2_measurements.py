...
SCRIPT = "code/v2/run_v2_measurements.py"
...
if "--help" in sys.argv or "-h" in sys.argv or "--dry-run" in sys.argv:
    ...
    for p in (CODE / "tc_pipeline.py", CODE / "datasetB_pipeline.py", DATA / "train.csv",
              DATA / "unique_m.csv", DATA / "datasetB_featurized.npz", DATA / "supercon_stanev.csv"):
        print(f"  {'ok ' if p.exists() else 'MISSING'} {p}")
    print("usage: python run_v2_measurements.py [--dry-run]   (full run: tens of minutes)")
    sys.exit(0)
...
dsB = load_mod("datasetB_pipeline", str(CODE / "datasetB_pipeline.py"))
...
# =============================================================== DATA
t_start = time.time()
featA = pd.read_csv(DATA / "train.csv"); uniqA = pd.read_csv(DATA / "unique_m.csv")
...
# superconductors_only=False reproduces the v1 (pre-mask factorize) codes on purpose: the
# alignment block below compares BOTH conventions against the npz. With the v2 default (True)
# gB_re would already be array_equal to the npz groups.
XB_re, yB_re, gB_re, formulas_all = dsB.featurize(str(DATA / "supercon_stanev.csv"), superconductors_only=False)
m_pos = yB_re > 0
...
json.dump(align, open(OUT / "verificacion_alineacion_B.json", "w"), indent=2, ensure_ascii=False)
...
pd.DataFrame(rows).to_csv(OUT / "nulo_presencia.csv", index=False)
...
pd.DataFrame(de_rows)[cols].to_csv(OUT / "design_effect.csv", index=False)
...
pd.DataFrame(seed_rows).to_csv(OUT / "seed_runs_top100.csv", index=False)
json.dump({f"{d}|{r}": v for (d, r), v in seed_store.items()},
          open(OUT / "seed_store.json", "w"), indent=2)
